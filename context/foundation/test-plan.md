...
Husky pre-push runs `npm run test:pre-push`, currently the unit suite via `npm run test`. The full handoff and CI gate remains `npx astro sync`, `npm run lint`, `npm run test`, and `npm run build`; do not treat typecheck, Stryker, API/UI/e2e, or build-on-push as required local hook gates until their own rollout explicitly ships them.
